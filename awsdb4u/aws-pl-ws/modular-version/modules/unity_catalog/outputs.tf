...
output "metastore_id" {
  description = "Unity Catalog metastore ID (created by Terraform)"
...
output "metastore_region" {
  description = "Unity Catalog metastore region"
...
output "root_storage_credential_name" {
  description = "UC root storage credential name (if created)"
  value       = var.create_workspace_catalog ? databricks_storage_credential.root_storage[0].name : null
}
...
output "root_storage_location_url" {
  description = "UC root storage location URL (if created)"
  value       = var.create_workspace_catalog ? databricks_external_location.root_storage[0].url : null
}
...
output "root_storage_iam_role_arn" {
  description = "UC root storage IAM role ARN (if created)"
  value       = var.create_workspace_catalog ? aws_iam_role.unity_catalog_root[0].arn : null
}
