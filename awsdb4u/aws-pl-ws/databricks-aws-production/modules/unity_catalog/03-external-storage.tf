...
# Step 7: Add KMS permissions for encrypted S3 buckets (inline policy)
# Only created when workspace catalog is enabled AND encryption is enabled
...
# Step 8: Wait for IAM propagation (includes KMS policy if encryption is enabled)
resource "time_sleep" "wait_for_uc_iam" {
  count = var.create_workspace_catalog ? 1 : 0

  create_duration = "60s" # Increased to 60s to ensure KMS policy propagation
  ...
  # Force recreation if KMS policy state changes
  triggers = {
...
resource "databricks_external_location" "external_location" {
  ...
  force_destroy   = true # Allow deletion even if dependent tables exist

  depends_on = [
    databricks_storage_credential.external_storage,
    time_sleep.wait_for_uc_iam,
    aws_iam_role_policy.external_location_kms # Explicit dependency on KMS policy
  ]
}
