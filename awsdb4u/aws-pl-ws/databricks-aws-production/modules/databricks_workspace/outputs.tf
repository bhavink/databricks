...
output "private_access_settings_id" {
  description = "MWS private access settings ID (existing or newly created, null if no VPC endpoints enabled)"
  value       = local.final_pas_id
}

output "private_access_settings_created" {
  description = "Whether a new Private Access Settings was created by this module (false if using existing PAS)"
