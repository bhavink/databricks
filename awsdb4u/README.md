...
Permissions: EC2 (instance, network, and volume lifecycle)
             IAM (CreateServiceLinkedRole, PutRolePolicy, for spot support only)
```

**No S3 permissions.** The cross-account role does not carry any `s3:*` actions.
Access to the workspace root bucket is granted by the Databricks-generated
**bucket policy** on the bucket itself, not by an IAM policy on this role. See
[AWS permissions reference](https://docs.databricks.com/aws/en/admin/cloud-configurations/aws/permissions).
