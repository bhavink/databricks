module "local_databricks" {
  # MODIFICATION: Phase-driven deployment using single variable
  # Reason: Simplified deployment - use terraform apply -var="phase=PROVISIONING" or -var="phase=RUNNING"
  expected_workspace_status = local.current_phase.expected_workspace_status
  # MODIFICATION: Safe provisioning flag ensures resources only created after workspace is RUNNING
  provision_workspace_resources = local.safe_provision_resources

  source = "../module"

  # Databricks Account Configuration - passed from root module variables
