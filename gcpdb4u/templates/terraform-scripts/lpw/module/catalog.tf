resource "databricks_storage_credential" "this" {
  ...
  ***REMOVED*** MODIFICATION: Changed to OPEN mode for explicit workspace binding
  ***REMOVED*** Reason: ISOLATED mode auto-binds and conflicts with explicit databricks_workspace_binding resources
  isolation_mode = "ISOLATION_MODE_OPEN"
  ***REMOVED*** MODIFICATION - This is what you used earlier
  ***REMOVED*** isolation_mode = each.value.shared == "false" ? "ISOLATION_MODE_ISOLATED" : each.value.shared == "true" ? "ISOLATION_MODE_OPEN" : "ISOLATION_MODE_OPEN"
...
resource "databricks_external_location" "external_location" {
  ...
  ***REMOVED******REMOVED*** Below code is creating a bucket, we can also pass the bucket url as a variables var.gs_bucket_url
  ***REMOVED***url = element(local.external_buckets_url, index(local.unity_catalog_name_list, each.value))
  ...
  ***REMOVED*** Reason: ISOLATED mode auto-binds and conflicts with explicit databricks_workspace_binding resources
  isolation_mode = "ISOLATION_MODE_OPEN"
  force_destroy  = true
  ***REMOVED*** MODIFICATION: Added IAM dependencies
  ***REMOVED*** Reason: Ensure GCS IAM permissions exist before Databricks validates bucket access
  ***REMOVED*** Added null_resource.wait_for_workspace_running dependency to make sure that workspace is in running state
  depends_on = [
...
resource "databricks_catalog" "workspace_catalog" {
  ...
  ***REMOVED***storage_root = element(local.external_buckets_url, index(local.unity_catalog_name_list, each.value))
  ...
  ***REMOVED***isolation_mode - (Optional) Whether the catalog is accessible from all workspaces or a specific set of workspaces. 
  ***REMOVED***Can be ISOLATED or OPEN. Setting the catalog to ISOLATED will automatically allow access from the current workspace.
  ***REMOVED*** MODIFICATION: Changed to OPEN mode for explicit workspace binding
  ***REMOVED*** Reason: ISOLATED mode auto-binds, we want explicit control via databricks_workspace_binding resources
  isolation_mode = "OPEN"
