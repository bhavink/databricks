...
***REMOVED*** gcloud auth activate-service-account token-creator@labs-byovpc-test.iam.gserviceaccount.com -key-file=token-creator-sa-creds.json
...
variable "databricks_delegate" {
 ...
 type        = list(string)
 ***REMOVED*** example of using a user identity with owner role on the project or a service account with tokenCreator role.
...
***REMOVED*** TODO: All these variables has be set to the proper values
variable "project" {
...
***REMOVED*** Service account for databricks workspace creation aka SA-2
