# PSC endpoints creation based on create_psc_endpoints variable
resource "google_compute_address" "relay_pe_ip_address" {
  for_each = var.create_psc_resources ? var.psc_attachments : {}

  name         = "${each.key}-relay-pe-ip" # Unique name per region
  provider     = google-beta
  project      = var.vpc_project_id # Updated to use project_id
  region       = each.key
  subnetwork   = google_compute_subnetwork.psc_subnets[each.key].id # Reference the PSC subnet
...
resource "google_compute_forwarding_rule" "relay_psc_ep" {
  ...
  depends_on = [
    google_compute_address.relay_pe_ip_address # Reference the specific address
  ]
  region                = each.key
  project               = var.vpc_project_id            # Updated to use project_id
  name                  = "${each.key}-relay-psc-ep"    # Unique name per region
  network               = google_compute_network.vpc.id # Reference to the VPC being created
...
resource "google_compute_address" "workspace_pe_ip_address" {
  ...
  name         = "${each.key}-workspace-pe-ip" # Unique name per region
  provider     = google-beta
  project      = var.vpc_project_id # Updated to use project_id
  region       = each.key
  subnetwork   = google_compute_subnetwork.psc_subnets[each.key].id # Reference the PSC subnet
...
resource "google_compute_forwarding_rule" "workspace_psc_ep" {
  ...
  depends_on = [
    google_compute_address.workspace_pe_ip_address # Reference the specific address
  ]
  region                = each.key
  project               = var.vpc_project_id             # Updated to use project_id
  name                  = "${each.key}-workspace-psc-ep" # Unique name per region
  network               = google_compute_network.vpc.id  # Reference to the VPC being created
  subnetwork            = google_compute_subnetwork.psc_subnets[each.key].id
