...
1. [Unity Catalog Destroy Issues](#unity-catalog-destroy-issues)
2. [Metastore Configuration](#metastore-configuration)
3. [Service Endpoint Policies](#service-endpoint-policies)
4. [NSG Rule Conflicts](#nsg-rule-conflicts)
5. [Authentication Issues](#authentication-issues)
6. [Storage Account Access](#storage-account-access)

---

## Unity Catalog Destroy Issues
...
resource "databricks_metastore" "this" {
  ...
  force_destroy = true  # CRITICAL: Allows clean destroy with root credential
...
### Issue: SEP Cannot Be Deleted (Attached to Subnets)
...
### Issue: NSG Rule Priority Conflicts with Databricks
...
export ARM_TENANT_ID="your-tenant-id"  # Also needed for Azure provider
```

**Provider Configuration**:
```hcl
# deployments/non-pl/providers.tf
provider "databricks" {
  alias             = "account"
...
export ARM_CLIENT_ID="..."        # If using service principal
export ARM_CLIENT_SECRET="..."    # If using service principal
...
resource "azurerm_storage_account" "metastore" {
  ...
  # Can be locked down post-deployment with firewall rules or SEP
}
```
...
# 2. If metastore errors occur, remove from state
terraform state rm 'module.unity_catalog.databricks_metastore_data_access.this[0]'
terraform state rm 'module.unity_catalog.databricks_metastore.this[0]'
terraform destroy -auto-approve

# 3. If SEP errors occur, update subnets first
terraform apply -target=module.networking.azurerm_subnet.public \
                -target=module.networking.azurerm_subnet.private \
                -auto-approve
terraform destroy -auto-approve
...
### 1. **Always Set `force_destroy = true` for Metastores**
```hcl
resource "databricks_metastore" "this" {
  # ...
...
resource "databricks_metastore" "this" {
  # ...
  depends_on = [
    azurerm_role_assignment.metastore_contributor  # Ensure RBAC is set
  ]
}
```
